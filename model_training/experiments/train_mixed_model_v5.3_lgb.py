...
MODEL_DIR   = "/home/master/wzheng/projects/model_training/models/v5.3_lgb"
REPORT_DIR  = "/home/master/wzheng/projects/model_training/reports/v5.3_lgb"
...
# ======= 新增：自适应内部留出（按样本量10~20%，min 500） =======
def make_internal_holdout_adaptive(df: pd.DataFrame,
                                   time_col: str,
                                   target_col: str,
                                   feature_cols: list,
    ...
    n = len(df_sorted)
    if n < 5_000:   ratio = max_ratio    # 小样本多留
    elif n < 50_000: ratio = 0.12
    else:            ratio = min_ratio    # 大样本少留
    n_ho = max(int(n * ratio), min_val_size)
    if n <= n_ho:
...
# ======= 新增：桶惩罚（用于 Optuna 目标） =======
def _bucketize(arr):
    return np.digitize(np.asarray(arr, dtype=float), _BUCKET_EDGES, right=False)

def bucket_penalized_rmse(y_true, y_pred, lam: float = 0.25):
    y_true = np.asarray(y_true, dtype=float); y_pred = np.asarray(y_pred, dtype=float)
    rmse = float(np.sqrt(((y_pred - y_true) ** 2).mean()))
    off = np.abs(_bucketize(y_pred) - _bucketize(y_true))   # 错几桶
    penalty = (off * lam * 1000).mean()                     # 线性惩罚（可调）
    return rmse + penalty
...
summary_rows = []
oof_path = os.path.join(REPORT_DIR, "oof_preds_v5.3.csv")
...
for pid in top_programids:
    ...
    enough_val = has_val and (len(df_val) >= 500) and (len(df_val) / max(len(df_train),1) >= 0.10)

    # 选择评估方式：优先“足量 Val”，否则“自适应内部留出”，否则“train-only”
    if enough_val:
        X_eval, y_eval = df_val[feature_cols], df_val[TARGET_COL]
        X_fit,  y_fit  = X_tr_full, y_tr_full
        eval_mode = "val"
    else:
        X_tr2, y_tr2, X_ho, y_ho = make_internal_holdout_adaptive(
            df=df_train, time_col=TIME_COL, target_col=TARGET_COL, feature_cols=feature_cols,
            min_val_size=500, min_ratio=0.10, max_ratio=0.20
        )
    ...
    if eval_mode in ("val", "internal_holdout"):
        # 用评估集做 Optuna（目标=RMSE+桶惩罚），早停仍用rmse
        def objective(trial):
            params = {
...
#                verbose=False,
            ...
            pred_eval = m.predict(X_eval, num_iteration=m.best_iteration_)
            return bucket_penalized_rmse(y_eval, pred_eval, lam=0.25)

        study = optuna.create_study(direction="minimize")
        study.optimize(objective, n_trials=N_TRIALS)

        best_params = study.best_params
        best_params.update({"n_estimators": 3000, "random_state": RANDOM_SEED})
...
#            verbose=False,
        ...
        y_eval_pred = final_model.predict(X_eval, num_iteration=final_model.best_iteration_)
        ov = overall_metrics(y_eval.values if hasattr(y_eval, "values") else y_eval, y_eval_pred)

        # 仅“有正式 val”时写分桶报表与准确率
        if eval_mode == "val":
        ...
        # 新增：持久化 OOF/Val 预测（给后续集成用）
        df_oof = pd.DataFrame({
            "ProgramID_encoded": pid,
            "y_true": np.asarray(y_eval if not hasattr(y_eval, "values") else y_eval.values),
            "y_pred": np.asarray(y_eval_pred),
            "model": "lgb_v5.3",
            "Status": eval_mode
        })
        df_oof.to_csv(oof_path, index=False, mode="a", header=not os.path.exists(oof_path))
...
sum_path = os.path.join(REPORT_DIR, "v5.3_lgb_evaluation_summary.csv")
